...
public class SearchingClass {
	...
	//@Produces("text/plain")
	public Response searchingTours (
			...
			@FormParam("type_of_hotel_room") String type_of_hotel_room) throws ParseException {
		...
		List<Tour> ansver = enterTour.selectToursByParam(sDate, eDate, enterCountry.selectById(end_Country), 
        		enterCity.selectById(end_City), enterHotel.selectById(hotel_id), 
        		price1, price2, hot_tour, stars, type_of_hotel_room, people);
		return Response.ok(ansver).build()//.getEntity().toString()
				;
	}
}
